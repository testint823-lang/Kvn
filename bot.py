...
LOGGER_GROUP_ID = int(os.getenv("LOGGER_GROUP_ID", "0"))  # Now using GROUP ID
...
user_clients = {}
report_data = {}
logger_group_invite_link = None  # Will be generated by bot
...
async def check_bot_admin_status():
    """Check if bot is admin in the logger group"""
    try:
        member = await bot.get_chat_member(LOGGER_GROUP_ID, bot.me.id)
        if member.status in ["administrator", "creator"]:
            # Check if bot has invite_users permission
            if member.privileges and member.privileges.can_invite_users:
                return True
            else:
                print("⚠️  Bot is admin but doesn't have 'Invite Users' permission!")
                return False
        return False
...
async def wait_for_admin_access():
    """Wait up to 1 minute for bot to be made admin with invite permission"""
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            # Check admin status with invite permission
            ...
            if is_admin:
                ...
                print(f"⚠️  Bot is not admin yet or missing permissions. Waiting 3 seconds...")
                await asyncio.sleep(3)
    ...
    print("⚠️  Please make bot admin with 'Invite Users' permission and restart!")
    print("=" * 60 + "\n")
    return False
...
async def generate_invite_link():
    ...
    try:
        ...
        invite_link = await bot.create_chat_invite_link(LOGGER_GROUP_ID)
        logger_group_invite_link = invite_link.invite_link
        
        print(f"✅ Invite link generated: {logger_group_invite_link}")
        return True
