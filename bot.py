...
async def start_command(client, message):
    ...
    await message.reply_text(
        ...
        f"• `/test_single` - Test single report with logging\n\n"
        f"**Quick Report:**\n"
...
@bot.on_message(filters.command("test_single") & filters.private)
@owner_only
async def test_single_report(client, message):
    """Test single report with detailed logging"""
    
    if not user_clients:
        await message.reply_text("❌ No active accounts!")
        return
    
    await message.reply_text(
        "🧪 **Single Report Test Mode**\n\n"
        "This will send ONE test report with full API logging.\n\n"
        "Reply to a message link with this command."
    )
    ...
    try:
        ...
        try:
            ...
            await message.reply_text(
                ...
                f"• Message ID: {msg_id}\n"
                f"• Link: `{target}`\n\n"
                f"**Will Report As:** Child Sexual Abuse (with comment)\n"
            ...
            await status_msg.edit_text(
                ...
                f"Assistant #{test_acc_num} could not access the message.\n\n"
                f"**Error:** `{str(e)[:100]}`"
            )
...
@bot.on_message(filters.private & ~filters.command(["start", "stats", "report", "check", "test_single"]))
@owner_only
